package org.dragon.tool.runtime.tools.browser.routes;

import io.netty.handler.codec.http.FullHttpRequest;
import io.netty.handler.codec.http.QueryStringDecoder;
import org.dragon.tool.runtime.tools.browser.PlaywrightSession;
import org.dragon.tool.runtime.tools.browser.server.DualChannelBridge;
...
public final class SnapshotRoutes {
    ...
    public static Object handleSnapshot(RouteContext ctx, QueryStringDecoder decoder,
        ...
        if ("role".equals(refsMode)) {
            var options = org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot.RoleSnapshotOptions.builder()
                    .interactive(interactive)
                    .compact(compact)
                    .maxDepth(maxDepth)
                    .build();
            var roleResult = org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot
                    .buildFromAriaSnapshot(snap.getSnapshot(), options);
            var stats = org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot
            ...
            org.dragon.tool.runtime.tools.browser.playwright.PageState.storeRoleRefs(
                    page, refsAsMap, "role", null);

            String snapshotText = roleResult.getSnapshot();
            if (maxChars > 0) {
                var tr = org.dragon.tool.runtime.tools.browser.snapshot.AiSnapshotFormatter.truncate(snapshotText, maxChars);
            ...
            if (maxChars > 0) {
                var tr = org.dragon.tool.runtime.tools.browser.snapshot.AiSnapshotFormatter.truncate(ariaText, maxChars);
    ...
    public static Object handleScreenshotWithLabels(RouteContext ctx, FullHttpRequest request,
        ...
        Map<String, org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot.RoleRef> refs = null;
        if (reqBody != null && reqBody.get("refs") != null) {
            refs = (Map<String, org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot.RoleRef>) reqBody.get("refs");
        } else {
            // Use stored refs from last snapshot
            var page = session.resolveTargetPage(targetId);
            var stored = org.dragon.tool.runtime.tools.browser.playwright.PageState.getStoredRoleRefs(page);
            if (stored != null) {
                refs = (Map<String, org.dragon.tool.runtime.tools.browser.snapshot.RoleSnapshot.RoleRef>)
                        (Map<String, ?>) stored;
            }
        }
