...
public final class BrowserProfiles {
     ...
     */
    public static Integer allocateCdpPort(Set<Integer> usedPorts, PortRange range) {
        if (range == null) {
            int rangeStart = BrowserConstants.DEFAULT_CDP_PORT_RANGE_START;
            int rangeEnd = rangeStart + BrowserConstants.DEFAULT_CDP_PORT_RANGE_SIZE - 1;
            range = new PortRange(rangeStart, rangeEnd);
        }
