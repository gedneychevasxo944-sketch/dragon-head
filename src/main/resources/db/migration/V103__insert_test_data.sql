...
-- 插入技能
INSERT INTO skills (id, name, description, category, visibility, tags, creator_type, creator_id, creator_name, status, created_at) VALUES
('1', '代码审查', '自动审查代码中的潜在问题并提供优化建议', 'CODER', 'PUBLIC', '["代码审查", "质量", "自动化"]', 'personal', 1, 'haha', 'DRAFT', NOW());

-- 插入技能版本 v1 (草稿)
INSERT INTO skill_versions (skill_id, version, name, description, content, frontmatter, runtime_config, editor_id, editor_name, status, storage_type, storage_info, created_at) VALUES
...
-- 插入技能版本 v2 (已发布)
INSERT INTO skill_versions (skill_id, version, name, description, content, frontmatter, runtime_config, editor_id, editor_name, status, release_note, storage_type, storage_info, created_at, published_at) VALUES
...
-- 更新技能的发布版本ID和状态
UPDATE skills SET status = 'ACTIVE', published_version_id = (SELECT id FROM (SELECT id FROM skill_versions WHERE skill_id = '1' AND version = 2) AS t) WHERE id = '1';
